...
export function PluginWorkspace() {
  ...
  const isLiveRun = Boolean(sessionId && sessionId.startsWith("r_"));
  ...
  const onStartRun = async (workflowId?: string) => {
    ...
    try {
      ...
      navigate(`/workspaces/${manifestQuery.data.id}/sessions/${run.id}`);
    } catch (e: unknown) {
  ...
  return (
    <AppShell
      ...
      crumbs={[
        ...
        sessionId
          ? { label: isLiveRun ? `Run ${sessionId}` : sessionId, active: true }
