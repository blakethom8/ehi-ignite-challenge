...
|---|---|---|---|
| 2026-05-12 | **AD** | Guest tier: upgrade PDF extraction to production multipass + fix export adapter | Guest now routes PDFs through `multipass-fhir` (same vision pipeline auth uses) instead of `pdfplumber-lab-text`. PDF/CCDA bundles persist to `outputs/per-source/<file_id>.fhir.json` so the export adapter splices PDF-extracted Encounters/Conditions/Meds/Immunizations/Observations into `evidence/canonical-facts.json` (previously orphaned in the per-run JSON). Page caps: per-run 60 (was 20), per-PDF 40, daily global circuit-break to lab-only fallback at 5000 pages. Missing `ANTHROPIC_API_KEY` triggers the same fallback. MANIFEST gets an `extraction` stanza; `patient_display` fallback is now `Unknown patient`. |
