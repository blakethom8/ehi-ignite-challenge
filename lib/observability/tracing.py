"""
LLM Observability & Tracing — canonical primitives.

Provides structured tracing for LLM/tool/retrieval calls anywhere in the
codebase. Stores traces locally in SQLite and optionally exports to
Langfuse.

This module is standalone: it does **not** import from ``api.*``. Config
is read lazily from environment variables so callers in ``api/``,
``lib/``, ``scripts/``, and ``ehi-atlas/`` all get the same behavior. The
env var names match the fields ``api.settings.Settings`` exposes, so
when the FastAPI app loads ``.env`` at boot the values flow through to
this module too.
...
# ---------------------------------------------------------------------------
# Configuration — lazy env var reads so tests can mutate env and see the
# change without reloading this module. ``api.settings`` reads the same
# names; the two stay in lock-step because they both ultimately consult
# ``os.environ``.
# ---------------------------------------------------------------------------
...
def traces_db_path() -> Path:
    return _env_path("TRACES_DB_PATH", "data/traces.db")


def tracing_sample_rate() -> float:
    return _env_float("TRACING_SAMPLE_RATE", 1.0, lo=0.0, hi=1.0)


def langfuse_public_key() -> str:
    return _env_str("LANGFUSE_PUBLIC_KEY")


def langfuse_secret_key() -> str:
    return _env_str("LANGFUSE_SECRET_KEY")


def langfuse_host() -> str:
    return _env_str("LANGFUSE_HOST", "https://cloud.langfuse.com")
...
def _provider_assistant_model() -> str:
    # Only used for the Langfuse ``model`` field on LLM generations. Falls
    # back to the same default api.settings exposes so behavior is
    # unchanged across the move.
    return _env_str("PROVIDER_ASSISTANT_MODEL", "claude-sonnet-4-5")
...
def start_span(
    ...
    if input_data is not None:
        try:
            input_json = json.dumps(input_data, default=str)[:50_000]
...
# ---------------------------------------------------------------------------
# SQLite Storage — init cache keyed by db path so tests pointing at a
# different tmp file each get a fresh schema bootstrap.
# ---------------------------------------------------------------------------
...
def _ensure_db(db_path: Path) -> None:
    """Create tables and indexes for ``db_path`` if they don't exist."""
    key = str(db_path)
...
# ---------------------------------------------------------------------------
# Langfuse Export — client cache keyed by (public_key, host) so changing
# credentials between tests doesn't pin a stale client.
# ---------------------------------------------------------------------------

_langfuse_clients: dict[tuple[str, str], Any] = {}
_langfuse_init_lock = threading.Lock()


def _get_langfuse():
    """Lazy-initialize a Langfuse client for the current env vars."""
    ...
    client = _langfuse_clients.get(cache_key)
    ...
    with _langfuse_init_lock:
        ...
        try:
            ...
            _langfuse_clients[cache_key] = client
        ...
        for span in trace.spans:
            if span.kind == SpanKind.LLM:
                lf_trace.generation(
                    ...
                    model=_provider_assistant_model(),
                    usage={
