...
| H0.12 | follow-on from H0.2 | **Production secrets runbook.** H0.2 made `ATLAS_SIGNING_KEY` and `EHI_SESSION_SECRET` required in production. There is no operator doc explaining how to generate them, set them, or what happens at boot if they're missing. Write `docs/architecture/PROD-SECRETS.md`: required env vars, generation commands (one-liner using `api/trust/signatures.generate_keypair`), rotation procedure, what each missing var fails at. | `docs/architecture/PROD-SECRETS.md` (new) | doc exists, includes copy-paste-ready key generation snippet that round-trips through `private_key_from_b64` | S |
